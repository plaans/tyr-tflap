...
string OpPreference::toString(const vector<Variable> &opParameters, ParsedTask* task) {
    return "(" + name + ": " + preference.toString(opParameters, task->functions,
...
string OpEquality::toString(const vector<Object> &objects) {
    string s = "(";
    if (value1.isVariable) s += "?" + to_string(value1.index);
    else s += objects[value1.index].name;
    if (equal) s += " = ";
    else s += " != ";
    if (value2.isVariable) s += "?" + to_string(value2.index);
...
string OpFluent::toString(const vector<Function> &functions, const vector<Object> &objects) {
    ...
    for (unsigned int i = 0; i < variable.params.size(); i++) {
        ...
        if (term.isVariable) s += " ?" + to_string(term.index);
        else s += " " + objects[term.index].name;
    }
    s += " = ";
    if (value.isVariable) s += " ?" + to_string(value.index);
...
string OpNumericPrec::toString(const vector<Function> &functions, const vector<Object> &objects) {
    string s;
    switch (comparator) {
    case CMP_EQ:        s = "(=";  break;
