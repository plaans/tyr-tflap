...
bool Preprocess::setParameterValues(unsigned int paramValues[], unsigned int equivalences[], const std::vector<OpEquality> &equality) {
    ...
    for (unsigned int i = 0; i < equality.size(); i++) {
        if (equality[i].equal) {
            ...
            if (equality[i].value1.isVariable && !equality[i].value2.isVariable) {          // (= ?p X)
                if (paramValues[p1] != MAX_UNSIGNED_INT && paramValues[p1] != p2) return false;
                paramValues[p1] = p2;
            } else if (equality[i].value2.isVariable && !equality[i].value1.isVariable) {   // (= X ?p)
                if (paramValues[p2] != MAX_UNSIGNED_INT && paramValues[p2] != p1) return false;
                paramValues[p2] = p1;
            } else if (equality[i].value1.isVariable) {                                     // (= ?p1 ?p2)
...
bool Preprocess::checkEqualities(unsigned int paramValues[], unsigned int equivalences[], 
    ...
    for (unsigned int i = 0; i < equality.size(); i++) {
        ...
        if (!equality[i].equal) {
            ...
            if (equality[i].value1.isVariable && equality[i].value2.isVariable) {          // (= ?p1 ?p2)
                if (paramValues[p1] != MAX_UNSIGNED_INT && paramValues[p2] != MAX_UNSIGNED_INT
                    && paramValues[p1] != paramValues[p2]) return false;
            }
        }
