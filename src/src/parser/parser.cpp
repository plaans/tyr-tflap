...
NumericExpression Parser::parseNumericExpression(const vector<Variable> &parameters) {
...
// <p-effect> ::=:object-fluents assign <function-term> <term>
// <p-effect> ::=:object-fluents assign <function-term> undefined
FluentAssignment Parser::parseFluentAssignment(const vector<Variable> &parameters) {
...
EffectExpression Parser::parseEffectExpression(const vector<Variable> &parameters, bool isAssign) {
    Token* token = syn->readSymbol(5, Symbol::NUMBER, Symbol::OPEN_PAR,
    ...
    case Symbol::NAME: 
        ...
        else if (!isAssign) syn->notifyError("Invalid assignment to 'undefined'");
        else if (token->description.compare("undefined") == 0) exp.type = EE_UNDEFINED;
        else {
            exp.type = EE_TERM;
            syn->undoToken();
            exp.term = parseTerm(parameters);
...
// <f-exp-t> ::= * <f-exp> #t
// <f-exp-t> ::= * #t <f-exp>
void Parser::parseEffectOperation(EffectExpression &exp, const vector<Variable> &parameters, bool isAssign) {
...
// <pref-GD> ::= <GD>
Precondition Parser::parsePrecondition(const vector<Variable> &parameters) {
    Precondition prec;
    syn->openPar();
...
void Parser::parseConstraintGoal(Constraint &constraint, const vector<Variable> &parameters) {
    GoalDescription g;
    g.time = NONE;
