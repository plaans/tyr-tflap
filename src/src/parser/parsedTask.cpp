...
string Term::toString(const vector<Variable> &parameters, const vector<Object> &objects) {
    if (isVariable) return parameters[index].name;
    else return objects[index].name;
}
...
string Literal::toString(const vector<Variable> &parameters, const vector<Function> &functions,
...
string NumericExpression::toString(const vector<Variable> &parameters, const vector<Function> &functions,
    const vector<Object> &objects) {
...
string Duration::toString(const vector<Variable> &parameters, const vector<Function> &functions,
    const vector<Object> &objects) {
    string s = "(";
...
string GoalDescription::toString(const vector<Variable> &opParameters, const vector<Function> &functions, 
       const vector<Object> &objects, const vector<Type> &taskTypes) {
    string s;
    switch (time) {
    ...
    switch (type) {
    case GD_LITERAL: s += literal.toString(opParameters, functions, objects); break;
    case GD_AND:     s += "(AND";
    ...
    case GD_INEQUALITY:
        s += "(!= " + eqTerms[0].toString(opParameters, objects) + " " +
            eqTerms[1].toString(opParameters, objects) + ")";
        break;
    case GD_NEG_LITERAL: s += "~" + literal.toString(opParameters, functions, objects); break;
    }
    return s;
}
...
string DurativeCondition::toString(const vector<Variable> &opParameters, const vector<Function> &functions,
       const vector<Object> &objects, const vector<Type> &taskTypes) {
    string s = "(";
    if (type == CT_AND) {
        s += "AND";
        ...
        s += "PREFERENCE " + preferenceName + "(" + 
            goal.toString(opParameters, functions, objects, taskTypes) + ")";
    }
    return s + ")";
}
...
string AssignmentContinuousEffect::toString(const vector<Variable> &opParameters, 
        const vector<Function> &functions, const vector<Object> &objects) {
...
string DurativeAction::toString(const vector<Function> &functions, const vector<Object> &objects, 
    ...
    return s + "\n* CONDITION " + condition.toString(parameters, functions,
        objects, taskTypes) + "\n* EFFECT " + effect.toString(parameters, 
...
string Precondition::toString(const vector<Variable> &opParameters, const vector<Function> &functions,
       const vector<Object> &objects, const vector<Type> &taskTypes) {
    string s;
    switch (type) {
    case PT_LITERAL: 
...
string Effect::toString(const vector<Variable> &opParameters, const vector<Function> &functions,
       const vector<Object> &objects, const vector<Type> &taskTypes) {
    string s;
    switch (type) {
    case ET_LITERAL: 
...
string Action::toString(const vector<Function> &functions, const vector<Object> &objects, 
    ...
    return s + ")\n* PRECONDITION " + precondition.toString(parameters, functions,
        objects, taskTypes) + "\n* EFFECT " + effect.toString(parameters, 
        functions, objects, taskTypes);
}
...
string Constraint::toString(const vector<Variable> &opParameters, const vector<Function> &functions, 
        const vector<Object> &objects, const vector<Type> &taskTypes) {
    string s = "(";
    switch (type) {
    case RT_AND:
    ...
    case RT_FORALL:
        ...
        break;
    case RT_PREFERENCE:
        s += "PREFERENCE " + preferenceName + " " + terms[0].toString(opParameters, functions, objects, taskTypes);
        break;
    case RT_AT_END:
    ...
    case RT_SOMETIME:
        ...
        break;
    case RT_WITHIN: 
        s += "WITHIN " + to_string(time[0]) + " " + goal[0].toString(opParameters, functions, objects, taskTypes);
        break;
    ...
    case RT_ALWAYS_WITHIN:
        s += "ALWAYS-WITHIN " + to_string(time[0]) + " " + goal[0].toString(opParameters, functions, objects, taskTypes)
             + " " + goal[1].toString(opParameters, functions, objects, taskTypes);
        break;
    case RT_HOLD_DURING:
        ...
        break;
    case RT_HOLD_AFTER:
        s += "HOLD-AFTER " + to_string(time[0]) + " " + goal[0].toString(opParameters, functions, objects, taskTypes);
        break;
    case RT_GOAL_PREFERENCE:
        s += "PREFERENCE " + preferenceName + " " + goal[0].toString(opParameters, functions, objects, taskTypes);
        break;
    }
    return s + ")";
}
...
unsigned int ParsedTask::getObjectIndex(string const &name) {
    ...
    if (index == typesByName.end()) return MAX_UNSIGNED_INT;
...
unsigned int ParsedTask::addAction(string name, const vector<Variable> &parameters, 
